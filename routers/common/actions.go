...
func DownloadActionsRunAllJobLogs(ctx *context.Base, ctxRepo *repo_model.Repository, runID int64) error {
	...
	if !hasLogs {
		return util.NewNotExistErrorf("no logs found")
	}
	...
	ctx.Resp.Header().Set("Content-Disposition", fmt.Sprintf("attachment; filename=%s-logs.zip", workflowName))
	ctx.Resp.Header().Set("Content-Type", "application/zip")
	...
	for _, job := range runJobs {
		...
		filename := fmt.Sprintf("%v-%v.log", job.Name, task.ID)
		w, err := writer.Create(filename)
		if err != nil {
			return fmt.Errorf("Create zip entry: %w", err)
		}
		if _, err := io.Copy(w, reader); err != nil {
			return fmt.Errorf("Copy log to zip: %w", err)
		}
	}
